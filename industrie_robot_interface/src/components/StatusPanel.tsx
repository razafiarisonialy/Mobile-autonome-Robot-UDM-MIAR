...
export const StatusPanel: React.FC<StatusPanelProps> = ({ messages, onClear }) => {
  ...
  const getMessageClass = (text: string) => {
    if (text.startsWith('[OK]') || text.includes('MISSION_TERMINEE') || text.includes('✓')) {
      return 'log-success';
    }
    if (text.startsWith('[ERREUR]') || text.includes('MISSION_ECHEC') || text.includes('✗')) {
      return 'log-error';
    }
    if (
  ...
  return (
    <div className="status-panel">
      <div className="panel-header">
        <h3 className="panel-title">Terminal de Suivi</h3>
      ...
      <div className="terminal-body">
        {messages.length === 0 ? (
          <div className="terminal-empty">En attente de connexion...</div>
        ) : (
          messages.map((msg) => (
